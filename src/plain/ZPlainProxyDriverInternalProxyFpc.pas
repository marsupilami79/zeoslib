...
unit ZPlainProxyDriverInternalProxyFpc;
...
uses
  Classes, SysUtils, {$IFDEF WINDOWS}Windows, ActiveX,{$ELSE}Types,{$ENDIF}
  ZPlainProxyDriverIntf, zeosproxy;

  type TZDbcProxy = class(TInterfacedObject, IZDbcProxy{$IFDEF WINDOWS}, ISupportErrorInfo{$ENDIF})
    protected
      FService: IZeosProxy;
      FConnectionID: WideString;
      procedure CheckConnected;
      // this is necessary for safecall exception handling
      {$IFDEF WINDOWS}function InterfaceSupportsErrorInfo(const iid: TIID): HResult; stdcall;{$ENDIF}
    public
      // this is necessary for safecall exception handling
      {$IFDEF WINDOWS}function SafeCallException(ExceptObject: TObject; ExceptAddr: Pointer): HResult; override;{$ENDIF}
...
function GetInterface: IZDbcProxy;
...
begin
  raise Exception.Create('ExecuteCB is not implemented (yet)');
end;
...
function TZDbcProxy.GetPublicKeys(EndPoint: WideString): WideString; overload; {$IFNDEF NO_SAFECALL}safecall;{$ENDIF}
begin
  raise Exception.Create('GetPublicKeys(WideString): WideString is not implemented (yet).')
end;
...
function GetLastErrorStr: WideString;
begin
  Result := 'GetLastErrorStr is not correctly implemented yet.'
end;
